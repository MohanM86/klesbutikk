          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 pb-10">
            {heroData.map((city) => (
              <Link key={city.slug} href={`/${city.slug}`}
                className={`group relative rounded-lg p-5 min-h-[140px] overflow-hidden transition-all duration-200 ${
                ...
                <div className="relative">
                  <h2 className={`font-body text-lg font-extrabold mb-1 transition-colors ${
                    city.accent === 'light' ? 'text-white group-hover:text-accent' : 'text-white'
                  }`}>{city.name}</h2>
                  <p className={`font-body text-xs mb-3 ${
                    city.accent === 'dark' ? 'text-white/50' :
                    city.accent === 'orange' ? 'text-white/70' : 'text-white/50'
                  }`}>{city.fylke}</p>
      ...
      <section className="bg-surface border-t border-border">
        <div className="max-w-8xl mx-auto section-padding py-12 md:py-16">
          <div className="max-w-2xl">
            <h2 className="font-body text-lg font-extrabold text-white mb-3">Klesbutikker i alle norske byer</h2>
