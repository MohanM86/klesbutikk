...
function CityPill({ city }: { city: CityData }) {
  return (
    <Link href={`/${city.slug}`}
      className="group flex-shrink-0 flex items-center gap-3 bg-cream border border-border rounded-lg px-5 py-4 hover:border-accent hover:shadow-md transition-all duration-200">
      <div className="w-10 h-10 rounded-lg bg-accent-light flex items-center justify-center group-hover:bg-accent transition-colors duration-200">
