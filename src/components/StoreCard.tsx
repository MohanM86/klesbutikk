...
export default function StoreCard({ store, showCity = false }: { store: Store; showCity?: boolean }) {
  return (
    <Link href={'/butikk/' + store.slug} className="group block border border-border rounded-lg p-4 transition-all duration-150 hover:border-black">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="font-body text-[11px] text-slate">{store.adresse}, {store.postnummer} {showCity ? store.poststed : ''}</p>
        </div>
      </div>
    </Link>
  );
