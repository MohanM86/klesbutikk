...
export function getAllCities(): CityData[] {
  ...
  for (const s of stores) {
    ...
    if (!cityMap.has(key)) cityMap.set(key, { fylke: s.fylke, count: 0 });
    cityMap.get(key)!.count++;
  }
  return Array.from(cityMap.entries())
    .map(([name, data]) => ({ name, slug: slugify(name), fylke: data.fylke, storeCount: data.count }))
...
export function getAllFylker(): FylkeData[] {
  ...
  for (const s of stores) {
    if (!s.fylke) continue;
    if (!fylkeMap.has(s.fylke)) fylkeMap.set(s.fylke, { count: 0, cities: new Set() });
    const f = fylkeMap.get(s.fylke)!;
    f.count++;
    f.cities.add(s.poststed);
  }
  return Array.from(fylkeMap.entries())
    .map(([name, data]) => ({ name, slug: slugify(name), storeCount: data.count, cities: Array.from(data.cities).sort() }))
    .sort((a, b) => b.storeCount - a.storeCount);
}
...
export function getRelatedStores(store: Store, limit: number = 6): Store[] {
  return stores.filter((s) => s.slug !== store.slug && s.poststed === store.poststed).slice(0, limit);
}
